...
// Set page title
$pageTitle = "Blood Request History - Red Cross";
...
$_SESSION['page_title'] = 'Blood Request History';
...
try {
    $completedRequests = executeQuery("
        SELECT 
            ...
            r.referral_date,
            r.id as referral_id,
            (SELECT COUNT(*) FROM blood_requests br2 WHERE br2.patient_id = br.patient_id AND br2.organization_type = 'redcross') as patient_total_requests,
            (SELECT COALESCE(SUM(units_requested), 0) FROM blood_requests br3 WHERE br3.patient_id = br.patient_id AND br3.organization_type = 'redcross') as patient_total_units_requested,
            (SELECT COALESCE(SUM(units_requested), 0) FROM blood_requests br4 WHERE br4.patient_id = br.patient_id AND br4.organization_type = 'redcross' AND br4.status IN ('Approved', 'Completed')) as patient_total_units_approved
...
<?php if (isset($_SESSION['blood_request_message'])): ?>
    <?php 
        $msgType = htmlspecialchars($_SESSION['blood_request_message_type'] ?? 'success', ENT_QUOTES, 'UTF-8');
    ?>
    <div class="alert alert-<?php echo $msgType; ?> alert-dismissible fade show" role="alert">
        <?php echo htmlspecialchars($_SESSION['blood_request_message'], ENT_QUOTES, 'UTF-8'); ?>
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>
    <?php unset($_SESSION['blood_request_message'], $_SESSION['blood_request_message_type']); ?>
<?php endif; ?>
