...
if (!$resolvedPath || !file_exists($resolvedPath)) {
    ...
    if ($altResolved && file_exists($altResolved)) {
        ...
        echo '<html><body style="font-family:Arial;padding:20px;text-align:center;"><h3>File Not Found</h3><p>The document file could not be located on the server.</p><p style="color:#666;font-size:12px;">DB Path: ' . htmlspecialchars($request['request_form_path']) . '</p><p style="color:#666;font-size:12px;">Resolved: ' . htmlspecialchars($filePath) . '</p></body></html>';
        exit;
    }
}
