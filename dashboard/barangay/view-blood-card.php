...
// Fetch the blood card path - only if the request belongs to this barangay or was referred to this barangay
$request = getRow("
    SELECT br.blood_card_path, br.id, br.patient_id, br.barangay_id
...
if (!$request || !$request['blood_card_path']) {
    ...
    echo '<html><body style="font-family:Arial;padding:20px;text-align:center;"><h3>Blood Card Not Found</h3><p>The blood card document is not available or you do not have access to this document.</p></body></html>';
    exit;
}
...
if (!$resolvedPath || !file_exists($resolvedPath)) {
    ...
    if (!$found) {
        http_response_code(404);
        header('Content-Type: text/html');
        // Log error without exposing user-controlled data
        error_log("View Blood Card - File not found for request ID (sanitized)");
        echo '<html><body style="font-family:Arial;padding:20px;text-align:center;"><h3>File Not Found</h3><p>The document file could not be located on the server.</p><p style="color:#666;font-size:12px;">Please contact support if this file should exist.</p></body></html>';
        exit;
    }
}
