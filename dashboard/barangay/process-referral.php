...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($updateResult === false) {
            ...
            error_log("Warning: Failed to update referral_status for request_id: " . $requestId);
        }
        ...
        try {
            ...
            if ($patientInfo && !empty($patientInfo['phone'])) {
                ...
                if (!empty($patientPhone) && trim($patientPhone) !== '') {
                    ...
                    error_log('[BARANGAY_SMS] Sending referral SMS to patient ID: ' . $patientId . ', Phone: ' . substr($patientPhone, 0, 4) . '****');
                    
                    $smsResult = send_sms_sim800c($patientPhone, $smsMessage);
                    
                    if ($smsResult['success']) {
                        error_log('[BARANGAY_SMS] Referral SMS sent successfully to patient');
                    } else {
                        $smsError = $smsResult['error'] ?? 'Unknown error';
                        error_log('[BARANGAY_SMS] Failed to send referral SMS: ' . $smsError);
                    }
                }
            }
        } catch (Exception $smsEx) {
            error_log('[BARANGAY_SMS_ERR] Exception in referral SMS: ' . $smsEx->getMessage());
