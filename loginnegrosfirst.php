...
if (rand(1, 100) === 1) {
    cleanupSecurityDataNF();
}

// Check if user is already logged in
if (isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] === 'negrosfirst') {
    // Check session timeout
    if (!checkSessionTimeoutNF()) {
        session_destroy();
        header("Location: loginnegrosfirst.php?expired=1");
        exit;
    }
    header("Location: dashboard/negrosfirst/index.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($step === 'verify_otp') {
        ...
        if (empty($otp) || strlen($otp) !== 6) {
            ...
            if (verifyOTPNF($userId, $otp, 'negrosfirst', 'login')) {
                // OTP verified successfully
                $user = getRow("SELECT * FROM negrosfirst_users WHERE id = ?", [$userId]);
                
                if ($user) {
                    // Clear failed login attempts
                    clearFailedLoginAttemptsNF($userId, 'negrosfirst');
                    
                    // Set session
                    $_SESSION['user_id'] = $user['id'];
                    $_SESSION['user_name'] = $user['name'];
                    $_SESSION['role'] = 'negrosfirst';
                    $_SESSION['last_activity'] = time();
                    
                    // Track session activity
                    trackSessionActivityNF($user['id'], 'negrosfirst');
                    
                    // Remember me cookie
                    if (!empty($_POST['rememberMe'])) {
                        $exp = time() + 60*60*24*30;
                        $payload = base64_encode(json_encode(['id' => (int)$user['id'], 'role' => 'negrosfirst', 'exp' => $exp]));
                        $sig = hash_hmac('sha256', $payload, bin2hex(getAppSecret()));
                        $cookie = $payload . '.' . $sig;
                        $cookieName = 'REMEMBER_BBP_NEGROSFIRST';
                    ...
                    audit_log($user['id'], 'negrosfirst', 'login_success_otp', 'email=' . $user['email']);
                ...
                audit_log($userId, 'negrosfirst', 'otp_verification_failed', 'Invalid OTP entered');
            }
        }
    } else {
        // Login step - email and password
        // Rate limiting
        if (rate_limit_exceeded('login:negrosfirst', 10, 600)) {
            $error = 'Too many login attempts. Please wait a few minutes and try again.';
            audit_log(null, 'negrosfirst', 'rate_limit_exceeded', 'Too many login attempts from IP: ' . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
        } else {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                $table = 'negrosfirst_users';
                $user = getRow("SELECT * FROM $table WHERE email = ?", [$email]);
                
                if ($user) {
                    // Check if account is locked
                    if (isAccountLockedNF($user['id'], 'negrosfirst')) {
                        $lockout = getRow(
                        ...
                        audit_log($user['id'], 'negrosfirst', 'login_blocked_locked', 'email=' . $email);
                    } else if (password_verify($password, $user['password'])) {
                        // Password correct - generate and send OTP
                        $otp = generateOTPNF($user['id'], $user['email'], 'negrosfirst', 'login', 10);
                        
                        if ($otp && sendOTPEmailNF($user['email'], $user['name'], $otp)) {
                            // Store temp session data for OTP verification
                            $_SESSION['temp_user_id'] = $user['id'];
                            $_SESSION['temp_user_email'] = $user['email'];
                            $_SESSION['temp_user_name'] = $user['name'];
                            
                            audit_log($user['id'], 'negrosfirst', 'otp_sent', 'OTP sent to email: ' . $user['email']);
                            
                            // Redirect to OTP verification
                            header("Location: loginnegrosfirst.php?step=verify_otp");
                            exit;
                        } else {
                            $error = 'Failed to send OTP email. Please try again or contact support.';
                            audit_log($user['id'], 'negrosfirst', 'otp_send_failed', 'Failed to send OTP to: ' . $user['email']);
                        }
                    } else {
                        // Invalid password - record failed attempt
                        recordFailedLoginNF($user['id'], 'negrosfirst', 5, 15);
                        $error = "Invalid email or password. Please try again.";
                        audit_log($user['id'], 'negrosfirst', 'login_failed', 'email=' . $email . ', reason=invalid_password');
                    }
                } else {
                    // User not found - don't reveal this
                    $error = "Invalid email or password. Please try again.";
                    audit_log(null, 'negrosfirst', 'login_failed', 'email=' . $email . ', reason=user_not_found');
                }
            }
        }
    }
...
if (isset($_GET['resend_otp']) && isset($_SESSION['temp_user_id'])) {
    ...
    if ($user) {
        $otp = generateOTPNF($userId, $user['email'], 'negrosfirst', 'login', 10);
        if ($otp && sendOTPEmailNF($user['email'], $user['name'], $otp)) {
            $success = 'A new OTP has been sent to your email.';
            audit_log($userId, 'negrosfirst', 'otp_resent', 'OTP resent to: ' . $user['email']);
        } else {
...
// Page title
$pageTitle = "Negros First Login - Blood Bank Portal";
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        body {
            min-height: 100vh;
            background: linear-gradient(rgba(0, 0, 0, 0.7), rgba(0, 0, 0, 0.7)), url('imgs/headernf.jpg');
...
<body>
    <div class="login-card">
        <div class="text-center">
            <img src="assets/img/nflogo.png" alt="Negros First Logo" class="login-logo">
            <div class="login-title">Negros First Provincial Blood Center</div>
        ...
        <?php if ($step === 'verify_otp'): ?>
            ...
            <form method="POST" action="">
                ...
                <div class="text-center mt-3">
                    <a href="loginnegrosfirst.php?resend_otp=1" class="text-danger text-decoration-none">
                        <i class="bi bi-arrow-clockwise me-1"></i>Resend OTP
                    </a>
                </div>
                
                <div class="text-center mt-2">
                    <a href="loginnegrosfirst.php" class="text-muted text-decoration-none small">
    ...
    <div class="login-footer">
        &copy; <?php echo date('Y'); ?> Negros First Provincial Blood Bank Portal
    </div>
