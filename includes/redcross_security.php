 ...
 */
function isAccountLocked(int $userId, string $role = 'redcross'): bool {
    $lockout = getRow(
 ...
 */
function recordFailedLogin(int $userId, string $role = 'redcross', int $maxAttempts = 5, int $lockoutMinutes = 15): void {
    $lockout = getRow(
 ...
 */
function clearFailedLoginAttempts(int $userId, string $role = 'redcross'): void {
    updateRow(
 ...
 */
function generateOTP(int $userId, string $email, string $role = 'redcross', string $purpose = 'login', int $expiryMinutes = 10): ?string {
 ...
 * Verify OTP
 */
function verifyOTP(int $userId, string $otp, string $role = 'redcross', string $purpose = 'login'): bool {
    $otpRecord = getRow(
...
function sendOTPEmail(string $email, string $name, string $otp): bool {
    ...
    try {
        ...
        $fromName = getenv('MAIL_FROM_NAME') ?: 'Philippine Red Cross Blood Bank';
        $mail->setFrom($from, $fromName);
        $mail->addAddress($email, $name ?: $email);
        $mail->isHTML(true);
        $mail->Subject = 'Red Cross Login - One-Time Password (OTP)';
        $mail->Body = '<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #b31217;">Philippine Red Cross Blood Bank</h2>
            <p>Hello ' . htmlspecialchars($name ?: $email) . ',</p>
            <p>You have requested to log in to your Red Cross account. Please use the following One-Time Password (OTP) to complete your login:</p>
            ...
            <p style="color: #666; font-size: 12px;">This is an automated message from the Philippine Red Cross Blood Bank Portal.</p>
        </div>';
        $mail->AltBody = "Red Cross Login OTP\n\nHello {$name},\n\nYour One-Time Password (OTP) is: {$otp}\n\nThis OTP will expire in 10 minutes.\n\nIf you did not request this, please ignore this email.";
 ...
 */
function trackSessionActivity(int $userId, string $role = 'redcross'): void {
...
/**
 * Check session timeout (30 minutes default, extended for active users)
 */
function checkSessionTimeout(int $timeoutMinutes = 30): bool {
    if (session_status() === PHP_SESSION_NONE) {
        session_start();
    }
    ...
    // Update last activity FIRST to prevent premature timeouts
    // This ensures that if the user is actively using the system, their session stays alive
    $currentTime = time();
    $timeSinceActivity = $currentTime - $_SESSION['last_activity'];
    
    // Check if session has expired
    if ($timeSinceActivity > ($timeoutMinutes * 60)) {
        // Session expired - only destroy if truly inactive
        session_destroy();
        return false;
    }
